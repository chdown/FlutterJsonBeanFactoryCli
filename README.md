...
### 快速注解检测
- **预筛选机制** - 在进行完整 AST 解析前，先通过快速字符串搜索检测文件是否包含 `@JsonSerializable` 或 `@JSONField` 注解
- **避免无效解析** - 跳过不包含目标注解的文件，减少不必要的 AST 解析开销
- **并行检测** - 使用 `Future.wait` 并行检测多个文件

### 并行处理
- **并行文件扫描** - 同时处理多个 Dart 文件的注解检测
- **并行代码生成** - 同时生成多个实体的 `.g.dart` 文件
- **异步 I/O** - 使用异步文件读写操作，提升整体性能
